...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
	...
	private boolean mHasPageIconIndicator = true;
	// xmc.launcher.indicator
    ...
    void setCurrentPage(int currentPage) {
		...
		// xmc.launcher.indicator
		updatePageIconIndicator();
		if (getPageCount() <= 1) {
			hidePageIconIndicator(true);
		}
		// xcm.launcher.indicator
  ...
  //xmc.launcher.indicator
	// reference to Scroll Indicator module
	protected boolean isPageIconIndicatorEnabled() {
		return true;
	}

	protected PageIndicatorView getPageIconIndicator() {
		//Log.d(TAG, "getPageIconIndicator");

		if (mHasPageIconIndicator && mPageIndicatorView == null) {
			ViewGroup parent = (ViewGroup) getParent();
			mPageIndicatorView = (PageIndicatorView) (parent
					.findViewById(R.id.page_icon_indicator));
			
			//Log.d(TAG, "getPageIconIndicator mPageIndicatorView == null");

			mHasPageIconIndicator = mPageIndicatorView != null;
			if (mHasPageIconIndicator) {
				//Log.d(TAG, "getPageIconIndicator: visible");
	...
	protected void hidePageIconIndicator(boolean immediately) {
		 if (getChildCount() <= 1) return;

		if (!isPageIconIndicatorEnabled())
			return;

		//Log.d(TAG, "hidePageIconIndicator");

		getPageIconIndicator();

		if (mPageIndicatorView != null) {
//			mPageIndicatorView.updatePageIconIndicatorPosition();

			//Log.d(TAG, "hidePageIconIndicator valid");

			mPageIndicatorView.setVisibility(View.INVISIBLE);
			mPageIndicatorView.setAlpha(0f);

			mPageIndicatorView.invalidate();
		}
	}

	protected void showPageIconIndicator(boolean immediately) {
		//if (getChildCount() <= 1) return;

		if (!isPageIconIndicatorEnabled())
			return;

		//Log.d(TAG, "showPageIconIndicator");

		getPageIconIndicator();

		if (mPageIndicatorView != null) {
			//Log.d(TAG, "showPageIconIndicator valid");
			
			mPageIndicatorView.setVisibility(View.VISIBLE);
			mPageIndicatorView.setAlpha(1f);
		}
	}

	protected void flashPageIconIndicator(boolean animated) {
		//Log.d(TAG, "flashPageIconIndicator");

		showPageIconIndicator(!animated);
	}

	protected void updatePageIconIndicator() {//void code
		if (getChildCount() < 1)return;

		if (!isPageIconIndicatorEnabled())
			return;

		//Log.d(TAG, "updatePageIconIndicator");

		getPageIconIndicator();

		if (mPageIndicatorView != null) {
			//Log.d(TAG, "updatePageIconIndicator valid");

			mPageIndicatorView.setValue(getCurrentPage(), getPageCount());
			mPageIndicatorView.invalidate();
		}
	}
	//xmc.launcher.indicator
