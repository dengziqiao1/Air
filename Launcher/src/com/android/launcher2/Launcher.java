...
import com.fc.serviceif.StartService_IF;
import com.fce.ui.widget.SubscribeActivity;
import com.fce.ui.widget.Widget_Main_Music;
...
public final class Launcher extends Activity
	...
	// add by xmc 20160129
	private static Launcher mSelf = null;
	...
	public static final String LAUNCHER_OPEN_SUBSCRIBE = "com.systemui.action.OPENSUBSCRIBE"; // 订阅
	public static final String LAUNCHER_OPEN_OPENAPPS = "com.systemui.action.OPENSYSTEMUIAPP";// 应用
	...
	// We set the state in both onCreate and then onNewIntent in some cases, which
	// causes both
	// scroll issues (because the workspace may not have been measured yet) and
	// extra work.
	// Instead, just save the state that we need to restore Launcher to, and commit
	// it in onResume.
	...
	// Holds the page that we need to animate to, and the icon views that we need to
	// animate up
	...
	private Widget_Main_Music mWidgetSource;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSelf = this; // add by xmc 20160129
		...
		// If we are getting an onCreate, we can actually preempt onResume and unset
		// mPaused here,
		// this also ensures that any synchronous binding below doesn't re-trigger
		// another
		// LauncherModel load.
		mPaused = false;
		...
		// On large interfaces, we want the screen to auto-rotate based on the current
		// orientation
		unlockScreenOrientation(true);

		// ------------------add by xmc start service-------------------
		if(!StartService_IF.getInstance().isCarServiceStarted( this )){
			StartService_IF.getInstance().startCarService( this );
		}
		if(!StartService_IF.getInstance().isRadioServiceStarted( this )){
			StartService_IF.getInstance().startRadioService( this );
		}
		if(!StartService_IF.getInstance().isMediaServiceStarted( this )){
			StartService_IF.getInstance().startMediaService( this );
		}
		if(!StartService_IF.getInstance().isIflyServiceStarted( this )){
			StartService_IF.getInstance().startIflyService( this );
		}
		if(!StartService_IF.getInstance().isBTServiceStarted( this )){
			StartService_IF.getInstance().startBTService( this );
		}

		if (!isMediaServiceStarted(this)) {// open MediaService
			startMediaService(this);
		}
	}

	private void startMediaService(Context context) {
		Log.v(TAG, "Launcher---startMediaService");
		Intent intentStart = new Intent();
		intentStart.setAction("action.fc.MEDIASERVICE");
		intentStart.setPackage("com.fc.mediaservice");
		context.startService(intentStart);
	}

	public boolean isMediaServiceStarted(Context context) {
		...
		for (int i = 0; i < serviceList.size(); i++) {
			if (serviceList.get(i).service.getClassName().equals("com.fc.service.MediaService")) {
				isStarted = true;
				break;
			}
		}
	...
	private void checkForLocaleChange() {
		if (sLocaleConfiguration == null) {
			AsyncTask<Void, Void, LocaleConfiguration> execute = new AsyncTask<Void, Void, LocaleConfiguration>() {
				@Override
	 ...
	 * widgets that have a configuration step, this allows the proper animations to
	 * run after other transitions.
	 */
	private boolean completeAdd(PendingAddArguments args) {
		...
		// Before adding this resetAddInfo(), after a shortcut was added to a workspace
		// screen,
		// if you turned the screen off and then back while in All Apps, Launcher would
		// not
	...
	protected void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
		...
		// Exit spring loaded mode if necessary after cancelling the configuration of a
		// widget
	...
	private void completeTwoStageWidgetDrop(final int resultCode, final int appWidgetId) {
		...
		if (mDragLayer.getAnimatedView() != null) {
			mWorkspace.animateWidgetDrop(mPendingAddInfo, cellLayout, (DragView) mDragLayer.getAnimatedView(),
	...
	protected void onResume() {
		...
		mWidgetSource.onActivityResume();

		// Reset the pressed state of icons that were locked in the press state while
		// activities
		// were launching
		...
		// It is possible that widgets can receive updates while launcher is not in the
		// foreground.
		// Consequently, the widgets will be inflated in the orientation of the
		// foreground activity
		// (framework issue). On resuming, we ensure that any widgets are inflated for
		// the current
		// orientation.
		getWorkspace().reinflateWidgetsIfNecessary();

		// Again, as with the above scenario, it's possible that one or more of the
		// global icons
	...
	protected void onPause() {
		// NOTE: We want all transitions from launcher to act as if the wallpaper were
		// enabled
		// to be consistent. So re-enable the flag here, and we will re-disable it as
		// necessary
	 ...
	 * 
	 * if (hasFocus) { final InputMethodManager inputManager = (InputMethodManager)
	 * getSystemService(Context.INPUT_METHOD_SERVICE); WindowManager.LayoutParams lp
	 * = getWindow().getAttributes(); inputManager.hideSoftInputFromWindow(lp.token,
	 * 0, new android.os.ResultReceiver(new android.os.Handler()) { protected void
	...
	/**
	 * Given the integer (ordinal) value of a State enum instance, convert it to a
	...
	private void setupViews() {
		...
		mWidgetSource = (Widget_Main_Music) findViewById(R.id.widget_source);

		mLauncherView.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN);
		mWorkspaceBackgroundDrawable = new ColorDrawable(Color.TRANSPARENT); // getResources().getDrawable(R.drawable.workspace_bg);
		mBlackBackgroundDrawable = new ColorDrawable(Color.TRANSPARENT); // add by xmc Apps界面背景全透明
	...
	View createShortcut(ShortcutInfo info) {
		return createShortcut(R.layout.application, (ViewGroup) mWorkspace.getChildAt(mWorkspace.getCurrentPage()),
				info);
	}
	...
	private void completeAddAppWidget(final int appWidgetId, long container, int screen, AppWidgetHostView hostView,
		...
		// We have saved the position to which the widget was dragged-- this really only
		// matters
		...
		if (mPendingAddInfo.cellX >= 0 && mPendingAddInfo.cellY >= 0) {
			...
			// when dragging and dropping, just find the closest free spot
			int[] result = layout.findNearestVacantArea(touchXY[0], touchXY[1], minSpanXY[0], minSpanXY[1], spanXY[0],
		...
		if (!mRestoring) {
			...
			mWorkspace.addInScreen(launcherInfo.hostView, container, screen, cellXY[0], cellXY[1], launcherInfo.spanX,
	...
	private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_SCREEN_OFF.equals(action)) {
				...
				mUserPresent = true;
				updateRunning();
			} else if (action.equals(LAUNCHER_OPEN_SUBSCRIBE)) {//add by xmc 1215
				startActivity(new Intent(mSelf, SubscribeActivity.class));
			} else if (action.equals(LAUNCHER_OPEN_OPENAPPS)) {
				showAllApps(true);
			}
		}
	};
	...
	public void onAttachedToWindow() {
		...
		// add by xmc 1215
		filter.addAction(LAUNCHER_OPEN_SUBSCRIBE);
		filter.addAction(LAUNCHER_OPEN_OPENAPPS);
	...
	public void onWindowVisibilityChanged(int visibility) {
		...
		updateRunning();
		// The following code used to be in onResume, but it turns out onResume is
		// called when
		...
		if (mVisible) {
			...
			if (!mWorkspaceLoading) {
				...
				// layers on all the workspace pages, so that transitioning to Launcher from
				// other
				// apps is nice and speedy. Usually the first call to preDraw doesn't correspond
				// to
			...
			// When Launcher comes back to foreground, a different Activity might be
			// responsible for
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		// We close any open folder since it will not be re-opened, and we need to make
		// sure
	...
	public void onDestroy() {
		...
		// Disconnect any of the callbacks and drawables associated with ItemInfos on
		// the workspace
	...
	public void startGlobalSearch(String initialQuery, boolean selectInitialQuery, Bundle appSearchData,
		...
		// Set source to package name of app that starts global search, if not set
		// already.
	 ...
	 */
	void addAppWidgetFromDrop(PendingAddWidgetInfo info, long container, int screen, int[] cell, int[] span,
			int[] loc) {
		resetAddInfo();
		...
		if (hostView != null) {
			...
			// mAppWidgetManager.setBindAppWidgetPermission(this.getPackageName(), true);
			// //xmc add
			if (options != null) {
...
//        }
	...
	public void onBackPressed() {
		if (isAllAppsVisible()) {
			...
			// Back button is a no-op here, but give at least some feedback for the button
			// press
	...
	public void onClick(View v) {
		...
		if (tag instanceof ShortcutInfo) {
			...
			// if (FCWindowManager.getInstance().FilterFCApk(v, intent, tag) == false) //add
			// by xmc
			{
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		// clicking anywhere on the workspace causes the customization drawer to slide
		// down
		showWorkspace(true);
		return false;
	}
	...
	boolean startActivity(View v, Intent intent, Object tag) {
		...
		try {
			// Only launch using the new animation if the shortcut has not opted out (this
			// is a
			...
			Log.e(TAG,
					"Launcher does not have the permission to launch " + intent
							...
							+ "or use the exported attribute for this activity. " + "tag=" + tag + " intent=" + intent,
					e);
		}
		return false;
	}
	...
	private void handleFolderClick(FolderIcon folderIcon) {
		...
		// If the folder info reports that the associated folder is open, then verify
		// that
		// it is actually opened. There have been a few instances where this gets out of
		// sync.
		if (info.opened && openFolder == null) {
			Log.d(TAG, "Folder info marked as open, but associated folder is not open. Screen: " + info.screen + " ("
	...
	/**
	 * This method draws the FolderIcon to an ImageView and then adds and positions
	 * that ImageView in the DragLayer in the exact absolute location of the
	 * original FolderIcon.
	 */
	private void copyFolderIconToImage(FolderIcon fi) {
		...
		// The layout from which the folder is being opened may be scaled, adjust the
		// starting
		...
		// Just in case this image view is still in the drag layer from a previous
		// animation,
	...
	private void growAndFadeOutFolderIcon(FolderIcon fi) {
		...
		// Push an ImageView copy of the FolderIcon into the DragLayer and hide the
		// original
	 ...
	 * folder is animated relative to the specified View. If the View is null, no
	 * animation is played.
	 *
	...
	public boolean onLongClick(View v) {
		...
		// The hotseat touch handling does not go through Workspace, and we always allow
		// long press
		// on hotseat items.
	...
	void updateWallpaperVisibility(boolean visible) {
		...
		int wpflags = WindowManager.LayoutParams.FLAG_SHOW_WALLPAPER; // add by xmc Apps界面背景全透明
	 ...
	 * from center screen - from other screens - Back from all apps - from center
	 * screen - from other screens - Launch app from workspace and quit - with back
	 * - with home - Launch app from all apps and quit - with back - with home - Go
	 * to a screen that's not the default, then all apps, and launch and app, and go
	 * back - with back -with home - On workspace, long press power and go back -
	 * with back - with home - On all apps, long press power and go back - with back
	 * - with home - On workspace, power off - On all apps, power off - Launch an
	 * app and turn off the screen while in that app - Go back with home key - Go
	 * back with back key TODO: make this not go to workspace - From all apps - From
	 * workspace - Enter and exit car mode (becuase it causes an extra configuration
	 * changed) - From all apps - From the center workspace - From another workspace
	 */

	/**
	 * Zoom the camera out from the workspace to reveal 'toView'. Assumes that the
	 * view to show is anchored at either the very top or very bottom of the screen.
	 */
	private void showAppsCustomizeHelper(final boolean animated, final boolean springLoaded) {

		final View scrollIndicator = (View) (this.findViewById(R.id.paged_view_indicator));
		...
		if (animated) {
			...
			mStateAnimation.addListener(new AnimatorListenerAdapter() {
				...
				public void onAnimationEnd(Animator animation) {
					...
					// add by xmc 从workspace进入apps界面完成之后，需要还原workspace透明度
					if (mWorkspace != null)
			...
			if (workspaceAnim != null) {
				...
				// add by xmc 从workspace进入apps界面，增加淡出动画
				ObjectAnimator alphaAnim_ws = ObjectAnimator.ofFloat(mWorkspace, "alpha", 1f, 0f)
						.setDuration(fadeDuration);
				mStateAnimation.play(alphaAnim_ws);
				alphaAnim_ws = ObjectAnimator.ofFloat(mHotseat, "alpha", 1f, 0f).setDuration(fadeDuration);
				mStateAnimation.play(alphaAnim_ws);
				alphaAnim_ws = ObjectAnimator.ofFloat(mDockDivider, "alpha", 1f, 0f).setDuration(fadeDuration);
				mStateAnimation.play(alphaAnim_ws);
				alphaAnim_ws = ObjectAnimator.ofFloat(scrollIndicator, "alpha", 1f, 0f).setDuration(fadeDuration);
				mStateAnimation.play(alphaAnim_ws);
			}
	...
	public void onWindowFocusChanged(boolean hasFocus) {
		if (!hasFocus) {
			// When another window occludes launcher (like the notification shade, or
			// recents),
			// ensure that we enable the wallpaper flag so that transitions are done
			// correctly.
			updateWallpaperVisibility(true);
		} else {
	...
	void showWorkspace(boolean animated, Runnable onCompleteRunnable) {
		if (mState != State.WORKSPACE) {
			...
			// Show the search bar (only animate if we were showing the drop target bar in
			// spring
			// loaded mode)
			if (mSearchDropTargetBar != null) {
			...
			// We only need to animate in the dock divider if we're going from spring loaded
			// mode
	...
	/**
	 * Add an item from all apps or customize onto the given workspace screen. If
	 * layout is null, add to the current screen.
	 */
	...
	/**
	 * Maps the current orientation to an index for referencing orientation correct
	 * global icons
	 */
	...
	private boolean updateGlobalSearchIcon() {
		...
		if (activityName != null) {
			...
			// We disable both search and voice search when there is no global search
			// provider
			if (searchButtonContainer != null)
	...
	private boolean updateVoiceSearchIcon(boolean searchVisible) {
		...
		// We only show/update the voice search icon if the search icon is enabled as
		// well
		final SearchManager searchManager = (SearchManager) getSystemService(Context.SEARCH_SERVICE);
		ComponentName globalSearchActivity = searchManager.getGlobalSearchActivity();
	...
	private void updateAppMarketIcon() {
		...
		if (activityName != null) {
			...
			// We should hide and disable the view so that we don't try and restore the
			// visibility
			// of it when we swap between drag & normal states from IconDropTarget
			// subclasses.
	...
	private void updateAppMarketIcon(Drawable.ConstantState d) {
		// Ensure that the new drawable we are creating has the approprate toolbar icon
		// bounds
		Resources r = getResources();
	 ...
	 * in onResume.
	 *
	 * This needs to be called from incoming places where resources might have been
	 * loaded while we are paused. That is becaues the Configuration might be wrong
	 * when we're not running, and if it comes back to what it was when we were
	...
	public void bindItems(ArrayList<ItemInfo> shortcuts, int start, int end) {
		...
		// Get the list of added shortcuts and intersect them with the set of shortcuts
		// here
		...
		for (int i = start; i < end; i++) {
			...
			case LauncherSettings.Favorites.ITEM_TYPE_FOLDER:
				FolderIcon newFolder = FolderIcon.fromXml(R.layout.folder_icon, this,
						(ViewGroup) workspace.getChildAt(workspace.getCurrentPage()), (FolderInfo) item, mIconCache);
	...
	public void finishBindingItems() {
		...
		// If we received the result of any pending adds while the loader was running
		// (e.g. the
		...
		// Update the market app icon as necessary (the other icons will be managed in
		// response to
		...
		if (mVisible || mWorkspaceLoading) {
			...
			if (canRunNewAppsAnimation()) {
				// If the user has not interacted recently, then either snap to the new page to
				// show
				// the new-apps animation or just run them if they are to appear on the current
				// page
				if (willSnapPage) {
	...
	/**
	 * Runs a new animation that scales up icons that were added while Launcher was
	 * in the background.
	 *
	...
	public void bindAllApplications(final ArrayList<ApplicationInfo> apps) {
		...
		if (progressBar != null) {
			...
			// If we did not initialize the spinner in onCreate, then we can directly set
			// the
			// list of applications without waiting for any progress bars views to be
			// hidden.
	...
	private int mapConfigurationOriActivityInfoOri(int configOri) {
		...
		// Since the map starts at portrait, we need to offset if this device's natural
		// orientation
		// is landscape.
		int indexOffset = 0;
	...
	private Cling initCling(int clingId, int[] positionData, boolean animate, int delay) {
		...
		if (cling != null) {
			...
			if (animate) {
				cling.buildLayer();
				cling.setAlpha(0f);
				cling.animate().alpha(1f).setInterpolator(new AccelerateInterpolator()).setDuration(SHOW_CLING_DURATION)
	...
	private void dismissCling(final Cling cling, final String flag, int duration) {
		// To catch cases where siblings of top-level views are made invisible, just
		// check whether
	...
	public void showFirstRunWorkspaceCling() {
		...
		if (isClingsEnabled() && !mSharedPrefs.getBoolean(Cling.WORKSPACE_CLING_DISMISSED_KEY, true)
	...
	public void showFirstRunAllAppsCling(int[] position) {
		// Enable the clings only if they have not been dismissed before
		if (isClingsEnabled() && !mSharedPrefs.getBoolean(Cling.ALLAPPS_CLING_DISMISSED_KEY, true)) {
	...
	public Cling showFirstRunFoldersCling() {
		// Enable the clings only if they have not been dismissed before
		if (isClingsEnabled() && !mSharedPrefs.getBoolean(Cling.FOLDER_CLING_DISMISSED_KEY, true)) {
