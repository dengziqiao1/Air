 ...
 * 下拉通知栏
 */
public class TopDrawLayout extends ViewGroup {
     ...
     */
    public TopDrawLayout(Context context, AttributeSet attrs) {
     ...
     */
    public TopDrawLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TopDrawLayout, defStyle,
                0);

        int orientation = a.getInt(R.styleable.TopDrawLayout_direction, ORIENTATION_BTT);
        mVertical = (orientation == ORIENTATION_BTT || orientation == ORIENTATION_TTB);
        mBottomOffset = (int) a.getDimension(R.styleable.TopDrawLayout_bottomOffset, 0.0f);
        mTopOffset = (int) a.getDimension(R.styleable.TopDrawLayout_topOffset, 0.0f);
        mAllowSingleTap = a.getBoolean(R.styleable.TopDrawLayout_allowSingleTap, true);
        mAnimateOnClick = a.getBoolean(R.styleable.TopDrawLayout_animateOnClick, true);
        mInvert = (orientation == ORIENTATION_TTB || orientation == ORIENTATION_LTR);

        int handleId = a.getResourceId(R.styleable.TopDrawLayout_handle, 0);
        if (handleId == 0) {
            throw new IllegalArgumentException("The handle attribute is required and must refer " + "to a valid child.");
        }

        int contentId = a.getResourceId(R.styleable.TopDrawLayout_content, 0);
        if (contentId == 0) {
...
// 去掉handle 高度
            int height = heightSpecSize - mTopOffset;
        ...
        LogUtils.e("dzq", "重新绘制： " + getMeasuredHeight());
    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mTracking || mAnimating) {
            ...
            if (cache != null) {
                if (isVertical) {
                    if (mInvert) {
                        LogUtils.e("dzq", "绘制图片：" + handle.getTop());
                        canvas.translate(0,handle.getTop() - (getBottom() - getTop()) + mHandleHeight);
...
//                                handle.getTop() - (getBottom() - getTop()) + mHandleHeight, null);
                    ...
                    canvas.drawBitmap(cache, mInvert ? handle.getLeft() - cache.getWidth() :
                ...
                if (mInvert) {
                    canvas.translate(isVertical ? 0 :
                            ...
                            isVertical ?
                                    handle.getTop() - mTopOffset - mContent.getMeasuredHeight() :
                                    0);
                } else {
                    canvas.translate(isVertical ? 0 : handle.getLeft() - mTopOffset, isVertical ?
                            handle.getTop() - mTopOffset : 0);
                }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        LogUtils.e("fce", "onLayout:  " + handleTop);
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        LogUtils.e("fce", "onInterceptTouch： " + getMeasuredHeight());
        if (mLocked) {
            return false;
        }
        if ((getMeasuredHeight() == 720 && !isIsfase2)||isOpened()) {
            LogUtils.e("fce", "onInterceptTouchEventwweeeeeee： " + getMeasuredHeight());
            isIsfase2 = true;
    ...
    public boolean isIsfase1 = false;
    public boolean isIsfase2 = false;
    public boolean isIsfase3 = false;

    @Override
    public boolean dispatchTouchEvent(MotionEvent ev) {
        LogUtils.e("fce", "dispatchTouchEvent    " + ev.getY());
        if (ev.getY() < 50 && !isIsfase1) {
            isIsfase1 = true;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (getMeasuredHeight() == 720||isOpened()) {
            LogUtils.e("fce", "拦截 onTouchEvent :" + getMeasuredHeight());
            if (mTracking) {
                mVelocityTracker.addMovement(event);
                final int action = event.getAction();
                switch (action) {
    ...
    private void performFling(int position, float velocity, boolean always) {
        ...
        if (mExpanded) {
            ...
            if (always || (c1 || (c2 && c3))) {
                ...
                // us to retract. Animate back to the expanded position. so animate BACK to
                // expanded!
                mAnimatedAcceleration = -mMaximumAcceleration;
    ...
    private void moveHandle(int position) {
        ...
        if (mVertical) {
            if (position == EXPANDED_FULL_OPEN) {
                ...
                } else handle.offsetTopAndBottom(mTopOffset - handle.getTop());
                invalidate();
                ...
                handle.getHitRect(frame);
                region.set(frame);
                LogUtils.e("fce", "刷新移动区域：  " + frame.top + "  " + deltaY);
                region.union(frame.left, frame.top - deltaY, frame.right, frame.bottom - deltaY);
                region.union(0, frame.bottom - deltaY, getWidth(),
                        frame.bottom - deltaY + getMeasuredHeight());
                invalidate(region);
            }
        } else {
            if (position == EXPANDED_FULL_OPEN) {
                ...
                else handle.offsetLeftAndRight(mTopOffset - handle.getLeft());
                invalidate();
            } else if (position == COLLAPSED_FULL_CLOSED) {
                if (mInvert) handle.offsetLeftAndRight(mTopOffset - handle.getLeft());
                else
    ...
    private void prepareContent() {
        ...
        LogUtils.e("dzq",
                "视图创建了:  " + content.getDrawingCache().getAllocationByteCount() + "   " + getMeasuredHeight());
        ...
        if (mAnimating) {
            ...
            if (mInvert) {
                if (mAnimationPosition < mTopOffset) {
                    mAnimating = false;
                    closeDrawer();
                } else if (mAnimationPosition >= mTopOffset + (mVertical ? getHeight() :
                        getWidth()) - 1) {
                    mAnimating = false;
                    openDrawer();
                } else {
