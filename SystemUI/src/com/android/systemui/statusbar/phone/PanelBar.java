...
import com.android.systemui.statusbar.phone.air.AirActivity;
   ...
   public PanelBar(Context context) {
	...
	if(event.getAction() == MotionEvent.ACTION_UP) {  
         ...
         float dy = event.getY()-downY;
		   //防止是按下也判断
        if (dy>30) {	
			    LogUtils.e("fce","down yes: "+dy);
			 //mPanel.onTouchEvent(event); 拦截下拉通知栏
				 Intent intent = new Intent(context, AirActivity.class);
				 context.startActivity(intent);		
				 return true;
		}else{
			 LogUtils.e("fce","down no: ");
			  return true;
		}

	}	

    return mPanel == null || true;
