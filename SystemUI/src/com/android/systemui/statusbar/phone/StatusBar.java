...
import android.content.IntentFilter;
import android.content.BroadcastReceiver;
import com.android.systemui.statusbar.phone.air.RxBus;
import com.android.systemui.statusbar.phone.air.ApiSubscriberCallBack;
import com.android.systemui.statusbar.phone.air.SystemUITitleInfo;
...
public class StatusBar extends SystemUI implements DemoMode,
        ...
        ColorExtractor.OnColorsChangedListener, ConfigurationListener, View.OnClickListener {
	...
	private KeyButtonView keyButtonView,imgSubscribe,imghistory,imgYinyong; //系统返回  记录 订阅   应用 
	private TextView texTitle;
	private String titleContent;
    private final String SYSTEMUI_UP_DATA_TITLE = "com.android.systemui.up.data.statusbar.title";//更新标题广播
    private final String SYSTEMUI_UP_DATA_TITLE_CONTENT = "systemui_title_content"; 
	private final String SYSTEMUI_UP_DATA_TITLE_OPENSUBSCRIBE = "com.systemui.action.OPENSUBSCRIBE" ;// 订阅
    private final String SYSTEMUI_UP_DATA_TITLE_OPENSYSTEMUIAPP = "com.systemui.action.OPENSYSTEMUIAPP" ;// 打开应用
	private final String SYSTEMUI_UP_DATA_TITLE_LUNCHER1 = "com.launcher.action.ONELUNCHER"; //luncher 1
	private final String SYSTEMUI_UP_DATA_TITLE_LUNCHER2 = "com.launcher.action.TWOLUNCHER";//luncher 2
	private final String SYSTEMUI_UP_DATA_TITLE_LUNCHER3 = "com.launcher.action.THREELUNCHER";//luncher 3
    private SystemUITitleBroadcast systemUITitle; //广播
    private SystemUITitleInfo systemUITitleInfo;  // 通知信息类
	private AlphaOptimizedFrameLayout alphaOptimized;  //左侧通知图标
	private LinearLayout openLayoutShow;//订阅  记录  应用  是否显示
	private int flags = 0; //订阅  记录  应用 广播记录

    /**
     * 更新标题
	   注册广播
     */
	private void setTitleInit(Context context){
	  systemUITitle = new SystemUITitleBroadcast();
      systemUITitleInfo = new SystemUITitleInfo();
	  IntentFilter filter = new IntentFilter();
      filter.addAction(SYSTEMUI_UP_DATA_TITLE);
	  filter.addAction(SYSTEMUI_UP_DATA_TITLE_LUNCHER1);
	  filter.addAction(SYSTEMUI_UP_DATA_TITLE_LUNCHER2);
	  filter.addAction(SYSTEMUI_UP_DATA_TITLE_LUNCHER3);
     ...
     * 更新标题广播
     */
    public class SystemUITitleBroadcast extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent == null) {
                return;
            }
			LogUtils.e("fce","getBroadcast：  "+intent.getAction());
            if (SYSTEMUI_UP_DATA_TITLE.equals(intent.getAction())) {
                titleContent = intent.getStringExtra(SYSTEMUI_UP_DATA_TITLE_CONTENT);
				setTitleShow();
				systemUITitleInfo.setTitle(titleContent);
                RxBus.getInstance().toSend(systemUITitleInfo);
            }else if(SYSTEMUI_UP_DATA_TITLE_LUNCHER1.equals(intent.getAction())){
				flags = 0; 
                 texTitle.setVisibility(View.GONE);
                 //keyButtonView.setVisibility(View.INVISIBLE);
				 openLayoutShow.setVisibility(View.INVISIBLE);
				 alphaOptimized.setVisibility(View.VISIBLE);

			}else if(SYSTEMUI_UP_DATA_TITLE_LUNCHER2.equals(intent.getAction())){
				flags = 1; 
                 texTitle.setVisibility(View.GONE);
                 //keyButtonView.setVisibility(View.INVISIBLE);
				 openLayoutShow.setVisibility(View.VISIBLE);
				 alphaOptimized.setVisibility(View.GONE);
			}else if(SYSTEMUI_UP_DATA_TITLE_LUNCHER3.equals(intent.getAction())){
				 flags = 2; 
                 texTitle.setVisibility(View.GONE);	 
                // keyButtonView.setVisibility(View.INVISIBLE);
				 openLayoutShow.setVisibility(View.INVISIBLE);
   ...
   private void setTitle() {
        RxBus.getInstance().toFlowable(SystemUITitleInfo.class).subscribe(new ApiSubscriberCallBack<SystemUITitleInfo>() {
            @Override
            public void onPiteSuccess(SystemUITitleInfo info) {
				titleContent = info.getTitle();
				setTitleShow();
				texTitle.setText(titleContent);
            }

            @Override
            public void onPiteFailure(Throwable e) {
              LogUtils.e("fce", "systemui title: " + e.toString());
            }
        });
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
		  ...
		  titleContent = "systemUITitle";
           // setTitleShow();
           // titleContent = mContext.getResources().getString(R.string.systemui_menu_title);
            //texTitle.setText(titleContent);
				 texTitle.setVisibility(View.GONE);
                // keyButtonView.setVisibility(View.GONE);
			 break;
			 
		  case R.id.systemui_history: //历史

             backFinsh(187);
        
			break;
		  case R.id.systemui_subscribe: //订阅

               intent = new Intent(SYSTEMUI_UP_DATA_TITLE_OPENSUBSCRIBE);
               mContext.sendBroadcast(intent);
			

	           break;
		
		  case R.id.systemui_yinyong: //应用
		     keyButtonView.setVisibility(View.VISIBLE);
              intent = new Intent(SYSTEMUI_UP_DATA_TITLE_OPENSYSTEMUIAPP);
              mContext.sendBroadcast(intent);
			break;
     }
   ...
   /**
	...
	private void setTitleShow(){
	   if(titleContent.equals("systemUITitle")){
                    titleContent ="";
				    texTitle.setVisibility(View.GONE);
                   // keyButtonView.setVisibility(View.INVISIBLE);
					if(flags == 1){
   ...
   /**
     * 退出 systemui
     */
	private void backFinsh(int flas){
	       new Thread() {
                      public void run() {
                        try {
                            Instrumentation inst = new Instrumentation();
							inst.sendCharacterSync(flas);
                        } catch (Exception e) {
    ...
    protected void makeStatusBarView() {
        ...
        FragmentHostManager.get(mStatusBarWindow)
                .addTagListener(CollapsedStatusBarFragment.TAG, (tag, fragment) -> {
					...
					texTitle = mStatusBarView.findViewById(R.id.systemui_title);//标题
					keyButtonView = mStatusBarView.findViewById(R.id.system_back);//返回
					alphaOptimized = mStatusBarView.findViewById(R.id.notification_icon_area);//左侧通知图标
					openLayoutShow = mStatusBarView.findViewById(R.id.systemui_line);//订阅  记录  应用  是否显示
                   //imgSubscribe,imghistory,imgYinyong
				    imghistory = mStatusBarView.findViewById(R.id.systemui_history);//历史
					imgSubscribe = mStatusBarView.findViewById(R.id.systemui_subscribe);//订阅
                    imgYinyong = mStatusBarView.findViewById(R.id.systemui_yinyong);//应用


					imghistory.setOnClickListener(this);
                    imgSubscribe.setOnClickListener(this);
					imgYinyong.setOnClickListener(this);

					keyButtonView.setOnClickListener(this);
    ...
    protected View.OnTouchListener getStatusBarWindowTouchListener() {
        return (v, event) -> {
           ...
           if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			LogUtils.e("fce","getStatusBarWindowTouchListener up");
